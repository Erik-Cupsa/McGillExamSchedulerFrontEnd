...
const Searching = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://mcgill-exams-26fa9ab0fe41.herokuapp.com/api/v1/exam${className ? `?className=${encodeURIComponent(className)}` : ''}`)
      .then((response) => {
  ...
  return (
    <>
      ...
      {loading ? <Loader type="pacman" /> : null}
    </>
  );
};
